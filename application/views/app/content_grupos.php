                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <button data-toggle="modal" data-target="#modal-edicion" class="btn" id="nuevoGrupo"><i class="far fa-plus-square btn"></button></i>
                        </div>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                        <table id="tablaGrupo" class="table table-striped">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Grupo</th>
                    ...
                    <form id="formGrupo">
                        <div class="card-body">
                            <div class="form-group">
                                <label for="descripcion_grupo">Estado</label>
                                <input type="text" name="descripcion_grupo" class="form-control" id="descripcion_grupo" placeholder="Descripción">
                            </div>
                        </div>
                        <!-- /.card-body -->
...
<script>
   ...
   // var validationBk;
            ...
            columns: [{ //aqui se levanta el id de cada fila que se muestra en la tabla para armar los id de los elementos editar y borrar del campo aciones
                ...
                },
                {
                    ...
                    render: function() {
                        ...
                        html += '<button data-toggle=modal data-target=#modal-edicion class="editarEstado btn" id = "editar' + id_grupo + '">';
                        html += '<i class= "fas fa-pencil-alt"></i>';
                        html += '</button>';
                        html += '</div>';
                        html += '<div class = "container" style = "margin: 0">';
                        html += '<button class= "borrarGrupo btn" id= "borrar' + id_grupo + '">';
    ...
    $('#nuevoGrupo').on('click', function(e) {
    ...
    $('#tablaGrupo').on('click', '.editarEstado', function() {
        //console.log('click en edición de registro');
        id = this.id;
        id_grupo = id.substring(6); //el id de este elemento es editarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        //con estas lineas se levanta el id a editar
        filtros['id'] = id_grupo;
    });
    //evento que se desata al momento de hacer click en el icono de borrar registro
    $('#tablaGrupo').on('click', '.borrarGrupo', function() {
        //console.log('click en borrar');
        id = this.id;
        id_grupo = id.substring(6); //el id de este elemento es borrarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        ...
        $.ajax({
            ...
            Swal.fire({
                title: 'Desea eliminar el siguiente registro?' + data.descripcion_grupo,
                    ...
                    $.ajax({
                        ...
                        .done(function(response) {
                            ...
                            if (responseJP.code == 200) {
                                $('#tablaGrupo').DataTable().ajax.reload();
                                Toast.fire({
                                    type: 'success',
    ...
    $('#modal-edicion').on('show.bs.modal', function(e) {
        ...
        if (id_grupo != "") { //si el id no es vacio, quiere decir que es una edición del registro y no una inserción
            ...
            $.ajax({
                ...
                $('#descripcion_grupo').val(responseJP['data'][0].descripcion_grupo);
