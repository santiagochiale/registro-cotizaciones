    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Estado OCs</h4>
                    ...
                    <form id="formEstado">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="estado_oc" class="form-control" id="estado_oc" placeholder="Descripción">
                            </div>
                        </div>
                        <!-- /.card-body -->
...
<script>
   ...
   // var validationBk;
    ...
    function cargarDataTable() {
        ...
        $("#tablaEstado").DataTable({
            ...
            columns: [{ //aqui se levanta el id de cada fila que se muestra en la tabla para armar los id de los elementos editar y borrar del campo aciones
                    data: function(data) {
                        id_estado_oc = data['id_estado_oc'];
                        return data['id_estado_oc'];
                    }
                },
                {
                    data: 'estado_oc'
                },
                {
                    ...
                    render: function() {
                        ...
                        html += '<button data-toggle=modal data-target=#modal-edicion class="editarEstado btn" id = "editar' + id_estado_oc + '">';
                        html += '<i class= "fas fa-pencil-alt"></i>';
                        html += '</button>';
                        html += '</div>';
                        html += '<div class = "container" style = "margin: 0">';
                        html += '<button class= "borrarEstado btn" id= "borrar' + id_estado_oc + '">';
    ...
    $('#tablaEstado').on('click', '.editarEstado', function() {
        ...
        id = this.id;
        id_estado_oc = id.substring(6); //el id de este elemento es editarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        //con estas lineas se levanta el id a editar
        filtros['id'] = id_estado_oc;
    });
    ...
    $('#tablaEstado').on('click', '.borrarEstado', function() {
        ...
        id = this.id;
        id_estado_oc = id.substring(6); //el id de este elemento es borrarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        ...
        $.ajax({
            ...
            Swal.fire({
                title: 'Desea eliminar el siguiente registro?' + data.estado_oc,
    ...
    $('#modal-edicion').on('show.bs.modal', function(e) {
        //{"modelo":"EstadoModel","valores":{"id":"26","estado_oc":"tarjetas nuevas","id_grupo":"2","cod_sap":"0000"}}
        //console.log(id_estado_oc);
        var urlAjax = "<?php echo base_url() ?>crud/encontrar_registro";
        
        if (id_estado_oc != "") { //si el id no es vacio, quiere decir que es una edición del registro y no una inserción
            ...
            $.ajax({
                ...
                $('#estado_oc').val(responseJP['data'][0].estado_oc);
                /*$.each(responseJP['data'], function(key, valor) {
                    $("#id_grupo").append(`<option id=${valor.id_estado_oc} value=${valor.id_grupo}>${valor.descripcion_grupo}</option>`);
                });*/
    ...
    $('#formEstado').on('submit', function(e) {
        ...
        //id_estado_oc = ""; // aqui se selecciona el registro que se quiere modificar. Si esta vacio es una insersión de registro nvo
        //var valores = {};
