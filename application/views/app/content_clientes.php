                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <button data-toggle="modal" data-target="#modal-edicion" class="btn" id="nuevoCliente"><i class="far fa-plus-square btn"></button></i>
                        </div>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                    ...
                    <form id="formClientes">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="nombre_cliente" class="form-control" id="nombre_cliente" placeholder="Descripción">
                            </div>
                        </div>
                        <!-- /.card-body -->
...
<script>
   ...
   // var validationBk;
            ...
            columns: [{ //aqui se levanta el id de cada fila que se muestra en la tabla para armar los id de los elementos editar y borrar del campo aciones
                ...
                },
                {
                    ...
                    render: function() {
                        ...
                        html += '<button data-toggle=modal data-target=#modal-edicion class="editarCliente btn" id = "editar' + id_cliente + '">';
                        html += '<i class= "fas fa-pencil-alt"></i>';
                        html += '</button>';
                        html += '</div>';
                        html += '<div class = "container" style = "margin: 0">';
                        html += '<button class= "borrarCliente btn" id= "borrar' + id_cliente + '">';
    ...
    $('#tablaClientes').on('click', '.editarCliente', function() {
        ...
        id = this.id;
        id_cliente = id.substring(6); //el id de este elemento es editarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        //con estas lineas se levanta el id a editar
        filtros['id'] = id_cliente;
    });
    //evento que se desata al momento de hacer click en el icono de borrar registro
    $('#tablaClientes').on('click', '.borrarCliente', function() {
        //console.log('click en borrar');
        id = this.id;
        id_cliente = id.substring(6); //el id de este elemento es borrarxx (xx es el id del proceso) con esta linea se extrae solo el numero
        ...
        $.ajax({
            ...
            Swal.fire({
                title: 'Desea eliminar el siguiente registro?' + data.nombre_cliente,
    ...
    $('#modal-edicion').on('show.bs.modal', function(e) {
        ...
        if (id_cliente != "") { //si el id no es vacio, quiere decir que es una edición del registro y no una inserción
            ...
            $.ajax({
                ...
                $('#nombre_cliente').val(responseJP['data'][0].nombre_cliente);
                /*$.each(responseJP['data'], function(key, valor) {
                    $("#id_grupo").append(`<option id=${valor.id_cliente} value=${valor.id_grupo}>${valor.descripcion_grupo}</option>`);
                });*/
    ...
    $('#formClientes').on('submit', function(e) {
        ...
        //id_cliente = ""; // aqui se selecciona el registro que se quiere modificar. Si esta vacio es una insersión de registro nvo
        //var valores = {};
